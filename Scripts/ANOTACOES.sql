...
SELECT		
		...
		N.DATA AS DATA
FROM NOTA_FISCAL N
INNER JOIN ITEM_NOTA I ON (N.IDNOTA = ID_NOTA_FISCAL)
...
SELECT @DATA AS DATA, DATEPART(DAY, @DATA) AS DIA,
	...
	--ATUALIZANDO ESTACOES
						...
						IF @DATA BETWEEN CONVERT(CHAR(4),@ano) + '0321'
						AND CONVERT(CHAR(4),@ANO) + '0620'
							SET @ESTACAO = 'Outono'

						IF @DATA BETWEEN CONVERT(CHAR(4),@ano) + '0621'
						AND CONVERT(CHAR(4),@ANO) + '0922'
							SET @ESTACAO = 'Inverno'

						ELSE --@data between 21/12 E 20/03(VERAO)
							SET @ESTACAO = 'Verão'
